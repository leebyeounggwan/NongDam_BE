...
public interface ScheduleRepository extends JpaRepository<Schedule,Long> {
    @Query("Select this_ from Schedule this_ where this_.member.id = :memberId and (this_.endTime > :now or this_.startTime > :lastWeek) order by this_.endTime desc")
    ...
    @Query("Select this_ from Schedule this_ where this_.member.id = :memberId and month(this_.startTime) = :month and year(this_.startTime) = :year order by this_.endTime desc")
    public List<Schedule> findScheduleOfMonth(@Param("memberId") int memberId, @Param("year") int year, @Param("month") int month);
    @Query("Select this_ from Schedule this_ where this_.member.id = :memberId and this_.startTime between :start and :end order by this_.endTime desc")
